...
Looking at the table of contents, I see three issues: numbers in titles, asterisks (*), and extra whitespace around some titles. 

```{r}
table_of_contents <- quakersaints[190:260, ] |> 
  filter(text != "") |> 
  mutate(
    younger = str_starts(text, "\\*"),
    text = str_remove_all(text, "[[:digit:]]+|\\*"),
    text = str_replace(text, "_page_ vii", ""),
    text = trimws(text)
  )
...
The join is creating extra matches because some titles appear more than once, causing repeated entries in the merged dataset and affecting the structure of the data.
...
```{r}
if (!"row_number" %in% colnames(table_of_contents)) {
  table_of_contents <- table_of_contents |> mutate(row_number = row_number())
}
title_locations <- 
  quakersaints_trimmed |> 
...
quakersaints_trimmed <- quakersaints_trimmed |> 
  ...
  mutate(text_type = coalesce(text_type, "Text")) |>
  fill(title, section_type, younger)
```
...
earlier? 
Looking at the top 20 words, I notice that NA values appear in the list. To clean this up, I should drop NA values. I could have done this earlier by filtering out missing values before counting words, ensuring that only valid words are included in the results.
...
quakersaints_long |> 
  ...
  select(-n) |> 
  pivot_wider(names_from = section_type, values_from = proportion) -> qs_props_section_type
```
...
font. We could do this multiple times by copy and pasting and modifying 
the list of numbers. 
...
```{r }
preprocessed <- quakersaints_long |> 
  group_by(title) |> 
  count(word, sort = TRUE) |> 
  slice_head(n = 10) |> 
...
make_faceted_title <- function(chapters) {
  preprocessed |> 
    ...
    scale_y_reordered() +
    theme(strip.text = element_text(size = 9, color = "dark green"))
}

make_faceted_title(seq(28, 38, 2))
make_faceted_title(seq(40, 52, 2))
make_faceted_title(seq(54, 64, 2))
```
